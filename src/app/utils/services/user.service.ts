...
export class UsersService implements OnDestroy {
  ...
  getChatPartner(chat: Chat): User | undefined {
    if (this.currentUser) {
      if (chat.memberIDs[0] === this.currentUserID)
        return this.getUserByID(chat.memberIDs[1]);
  ...
  ifSelfChat(chat: Chat): boolean {
    if (this.currentUser)
      return (
        chat.memberIDs[0] === this.currentUser.id &&
        chat.memberIDs[1] === this.currentUser.id
      );
    return false;
  }
  ...
  private async addChatWithUserOnFirestore(
    userID: string
  ): Promise<Chat | undefined> {
    try {
      ...
      this.updateCurrentUserDataOnFirestore({
        chatIDs: [...(this.currentUser?.chatIDs || []), chat.id],
      });
      if (this.currentUserID !== userID)
        this.updateUserDataOnFirestoreByID(userID, {
  ...
  async updateCurrentUserDataOnFirestore(userChangeData: {
    ...
    if (this.currentUser) {
      await this.updateUserDataOnFirestoreByID(
        this.currentUserID,
        userChangeData
      );
    }
  ...
  private async updateUserDataOnFirestoreByID(
    id: string,
    userChangeData: {
      email?: string;
      name?: string;
  ...
  async updateCurrentUserEmail(
    ...
    try {
      ...
      if (auth.currentUser) {
        ...
        if (this.currentUser) {
          this.updateUserDataOnFirestoreByID(this.currentUser.id, {
            email: newEmail,
          });
        }
        return undefined;
